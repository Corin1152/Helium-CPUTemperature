...
# Xcode 自带的 iOS SDK 不含上面那几个私有框架的 stub，
# 构建时由 CI 从 theos/sdks 取来放进 private-frameworks/，这里加进搜索路径。
# 本地没有该目录时，clang 只是忽略这个无效路径，不影响正常编译。
$(APPLICATION_NAME)_LDFLAGS += -F$(shell pwd)/private-frameworks
